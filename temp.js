...
window.viewOldPOByNo = async () => {
        ...
        async function loadTransferOut() {
            ...
            const filtered = raw.docs.filter(d => {
                const data = d.data();
                return data.fromLoc === session.loc;
            });
                            ...
                            <tbody>
                                ${outSnap.docs.map(d => {
                                    ...
                                    <tr class="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td class="p-5">
                                             <span class="${color} px-3 py-1 rounded-full text-[10px] font-black uppercase border border-current opacity-80">${st === 'PENDING' ? 'DRAFT' : st}</span>
                                        </td>
                                        <td class="p-5 text-sm font-medium text-slate-500">${new Date(d.data().date).toLocaleDateString()}</td>
                                        <td class="p-5 text-right">
                                            <div class="flex justify-end gap-2">
                                                ${st === 'PENDING' ? `
                                                    <button onclick="editTransferOut('${d.id}')" class="bg-blue-100 text-blue-600 px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-blue-200 transition active:scale-95 shadow-sm border border-blue-200">Open Draft</button>
                                                ` : ''}
        ...
        window.createTransferOut = async (editId = null) => {
            const locs = await getDocs(collection(db, "locations"));
            const itemsSnap = await getDocs(collection(db, "items"));
            window.trItems = [];
            window.activeTrId = null;
            let destLoc = '';
            let isEdit = false;

            if (editId) {
                isEdit = true;
                const d = (await getDoc(doc(db, "docs", editId))).data();
                window.trItems = d.items || [];
                window.activeTrId = editId;
                window.activeTrNo = d.docNo;
                destLoc = d.toLoc;
            }

            const locOpt = locs.docs.filter(l => l.data().name !== session.loc).map(l => `<option ${destLoc === l.data().name ? 'selected' : ''}>${l.data().name}</option>`).join('');
            ...
            openModal(isEdit ? "Update Transfer Draft" : "Issue New Stock Transfer", `
                <div class="space-y-6">
                    <!-- Global Settings -->
                    ...
                    <div id="tr-actions" class="pt-4 flex gap-4">
                        <button onclick="createTrExec()" id="btn-create-tr" class="flex-1 bg-slate-900 border-2 border-slate-900 text-white p-5 rounded-2xl font-black text-xl hover:bg-white hover:text-slate-900 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3">
                             <span>Save Transfer Draft</span>
                             <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg>
                        </button>
                        ${isEdit ? `
                        <button onclick="triggerDispatch()" id="btn-create-dispatch" class="flex-1 bg-emerald-600 border-2 border-emerald-600 text-white p-5 rounded-2xl font-black text-xl hover:bg-emerald-700 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3">
            ...
            setTimeout(() => { loadTrBatches(); renderTrManifest(); }, 100);
        };

        window.editTransferOut = (id) => {
             createTransferOut(id);
        };

        window.triggerDispatch = async () => {
             const btn = document.getElementById('btn-create-dispatch');
             btn.innerHTML = '<span>Processing...</span>';
             await createTrExec(true);
        };
                ...
                `<div class="space-y-6">
                    ...
                    ${d.status === 'PENDING' ? `
                        <div class="bg-emerald-50 border border-emerald-200 p-6 rounded-3xl flex items-center justify-between">
                            ...
                            <button id="btn-view-complete-tr" onclick='window.activeTrId="${id}";window.activeTrNo="${d.docNo}";window.trItems=${JSON.stringify(d.items).replace(/'/g, "&#39;")};completeTrExec()' class="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl shadow-emerald-200 hover:scale-105 active:scale-95 transition-all text-sm">COMPLETE DISPATCH</button>
                        </div>
                    ` : ''}
                    ...
                    `<div class="flex gap-3 w-full">
                        <button onclick="window.printTransfer('${id}')" class="flex-1 bg-slate-900 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition shadow-lg">Print Disptach Note</button>
        ...
        window.createTrExec = async (completeDispatch = false) => {
            ...
            try {
                ...
                let id, dRef;
                if (window.activeTrId) {
                    id = window.activeTrNo;
                    dRef = doc(db, "docs", window.activeTrId);
                    await setDoc(dRef, {
                    ...
                    id = await generateID('TR-OUT', 'docs');
                    dRef = doc(collection(db, "docs"));
                    await setDoc(dRef, {
                        docNo: id, type: 'TRANSFER', fromLoc: session.loc, toLoc: dest,
                        items: cleanItems, status: 'PENDING', date: new Date().toISOString(),
                        user: session.user
                ...
                if (completeDispatch) {
                     return await completeTrExec();
                }

                alert(`Draft Transfer ${id} Saved Successfully!`);
                closeModal();
                loadTransferOut();
                ...
                alert("Error: " + e.message);
                btn.disabled = false;
                btn.innerHTML = '<span>Save Transfer Draft</span>';
            }
        };

        window.completeTrExec = async () => {
            const btn = document.getElementById('btn-create-dispatch') || document.getElementById('btn-view-complete-tr');
            if(btn) {
                btn.disabled = true;
                             ...
                             <div class="card p-2 bg-white border border-slate-200 divide-y divide-slate-100">
                                 ${hist.map(d => `
                                     <div class="p-4 hover:bg-slate-50 transition-colors group flex items-center justify-between gap-4">
                                         ...
                                         <button onclick="window.printTransfer('${d.id}')" class="p-2 rounded-lg hover:bg-emerald-50 text-emerald-500 transition-colors" title="Reprint Receipt">
            ...
            try {
                ...
                const rows = d.items.map((x, idx) => `
                    <tr>
                        ...
                        <td style="padding:12px; border-bottom:1px solid #eee;"><b>${x.name}</b><br><small>CD: ${x.code}</small></td>
                        <td style="text-align:center; font-weight:bold;">${x.qty}</td>
                        <td style="text-align:center;">${x.exp || 'N/A'}</td>
                    </tr>
                `).join('');

                const isIngoing = d.toLoc === session.loc;
                const titleStr = isIngoing ? "STOCK RECEIPT SUMMARY" : "STOCK DISPATCH NOTE";

                const w = window.open('', '', 'width=900,height=800');
                w.document.write(`
                    <html><head><title>${d.docNo}</title>
                    <style>
                        body { font-family: 'Segoe UI', Tahoma, sans-serif; padding: 40px; color: #1e293b; line-height: 1.6; }
                        ...
                        h1 { font-size: 30px; font-weight: 900; margin: 0; color: #10b981; letter-spacing: -1px; }
                        .co-details { text-align: left; }
                        .meta { display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 40px; background: #ecfdf5; padding: 25px; border-radius: 20px; border: 1px solid #a7f3d0; }
                        .meta-box h4 { margin: 0 0 5px; text-transform: uppercase; font-size: 10px; color: #10b981; letter-spacing: 1px; }
                        .meta-box p { margin: 0; font-weight: 800; font-size: 14px; }
                        table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
                        th { background: #10b981; color: white; padding: 12px; text-transform: uppercase; font-size: 11px; font-weight: 900; border: 1px solid #10b981; }
                        td { padding: 12px; border: 1px solid #f1f5f9; font-size: 13px; text-transform: uppercase; }
                        .footer { margin-top: 100px; border-top: 1px dashed #cbd5e1; padding-top: 20px; display: flex; justify-content: space-between; font-size: 10px; color: #94a3b8; text-transform: uppercase;}
                    </style>
                    </head><body>
                        <header>
                            <div class="co-details" style="display:flex; align-items:center; gap:15px;">
                                <img src="logo.png" style="height:60px; object-fit:contain;">
                                <div>
                                    <div style="font-size:38px; font-weight:900; color:#1e293b; line-height:1;">${c.name}</div>
                                    <div style="font-size:14px; font-weight:600; color:#64748b; margin-top:5px;">${c.addr} | ${c.tel}</div>
                                </div>
                            </div>
                            <div style="text-align:right;">
                                <h1 style="text-transform:uppercase;">${titleStr}</h1>
                                <p style="margin:5px 0; font-weight:700; color:#94a3b8; font-size:12px;">Internal Transfer Logistics</p>
                            </div>
                        </header>
                        <div class="meta">
                            <div class="meta-box"><h4>Transfer Ref #</h4><p>${d.docNo}</p></div>
                            <div class="meta-box"><h4>Status</h4><p>${d.status}</p></div>
                            <div class="meta-box"><h4>Origin Vault</h4><p>${d.fromLoc}</p></div>
                            <div class="meta-box"><h4>Destination Vault</h4><p>${d.toLoc}</p></div>
                        </div>
                        <table>
                            <thead><tr><th>#</th><th>Asset Description</th><th style="text-align:center">Transfer Qty</th><th style="text-align:center">Expiry Batch</th></tr></thead>
                            <tbody>${rows}</tbody>
                        </table>
                        <div class="footer">
                             <div>Authorized Node Signature: _______________________</div>
                             <div>Logistics Handled By ${d.user || session.user} | PSS - Stock System</div>
                        </div>
                    </body></html>
                `);
                w.document.close();
